...
public enum ModifierType {
    ...
    tech_weight_likelihood(DefaultParser.SCRIPTED),

    is_ai("Is AI|Is NOT AI", DefaultParser.SIMPLE_BOOLEAN),
    has_federation("Is in a federation|Is NOT in a federation"),
    is_galactic_community_member("Is a member of the Galactic Community|Is NOT a member of the Galatic Community"),
    ...
    is_lithoid_empire("Owner Species is Lithoid", DefaultParser.SIMPLE_BOOLEAN),
    ...
    is_robotic_species("Is Robotic|Is NOT Robotic", DefaultParser.SIMPLE_BOOLEAN),
    ...
    count_owned_pop("Number of Owned pop : ", DefaultParser.CONDITIONAL),
    ...
    is_specialist_subject_type((p) -> {
        String subject = p.value().map().pair().get(0).value().BAREWORD().toString();
        return "Is Subject Type : " + subject;
    }),
    ...
    private static enum DefaultParser {
        ...
        CONDITIONAL((format, p) -> {
            ...
            for(PairContext prop : p.value().map().pair()) {
                Modifier m = visitCondition(prop);
                if(m.type != factor) conditions.add(m.toString());
            }
